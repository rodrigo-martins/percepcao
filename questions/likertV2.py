...
def analyze_likert(df: pd.DataFrame, out_dir: Optional[Path] = None, language: str = "en") -> Dict[str, Any]:
    """
    ...
    Parâmetros:
    - language: "en" para inglês ou "pt" para português
    """
    # Dicionário de labels por idioma
    labels_dict = {
        "en": ["Strongly disagree", "Disagree", "Neutral", "Agree", "Strongly agree"],
        "pt": ["Discordo totalmente", "Discordo", "Neutro", "Concordo", "Concordo totalmente"]
    }
    order = labels_dict.get(language, labels_dict["en"])
    ...
    if language == "pt":
        ...
        manual_divisions = [
            ...
            ("Training Induction & Pretraining Environment", [13, 14]),
    ...
    # nota: 'order' já foi definida no início da função baseada no idioma selecionado

    # Se o idioma for inglês, converter os valores dos dados de português para inglês
    if language == "en":
        pt_labels = ["Discordo totalmente", "Discordo", "Neutro", "Concordo", "Concordo totalmente"]
        en_labels = ["Strongly disagree", "Disagree", "Neutral", "Agree", "Strongly agree"]
        translation_map = dict(zip(pt_labels, en_labels))
        
        # substituir valores em todas as colunas
        for col in df_likert.columns:
            df_likert[col] = df_likert[col].replace(translation_map)
    ...
    for neg in order[:3]:  # primeiros 3 itens: [Disagree/Strongly disagree, Disagree, Neutral]
    ...
    try:
        ...
        color_map = {
            order[0]: shades[1] if len(shades) > 1 else BASE_ORANGE,  # Strongly disagree / Discordo totalmente
            order[1]: shades[0] if len(shades) > 0 else BASE_ORANGE,  # Disagree / Discordo
            order[2]: (0.827, 0.827, 0.827),  # light gray - Neutral / Neutro
            order[3]: shades[-2] if len(shades) > 1 else BASE_ORANGE,  # Agree / Concordo
            order[4]: shades[-1] if len(shades) > 0 else BASE_ORANGE,  # Strongly agree / Concordo totalmente
        }

        # plot stacked horizontal divergent bars
        # Largura e altura dinâmicas baseadas no número de perguntas
        fig_width = max(12, 0.3 * len(counts))
        fig_height = max(14, 0.4 * len(counts))
        fig, ax = plt.subplots(figsize=(fig_width, fig_height))
        height = 0.70

        # negativo: order[0], order[1], order[2] (order[2] Neutral/Neutro fica à esquerda por escolha)
        neg_cols = [c for c in [order[2], order[1], order[0]] if c in percent.columns]
        ...
        # positivo: order[3], order[4] (Agree, Strongly agree / Concordo, Concordo totalmente)
        pos_cols = [c for c in [order[3], order[4]] if c in percent.columns]
        ...
        x_text_base = xmax + (xmax - xmin) * 0.02
        right_margin = x_text_base + (xmax - xmin) * 0.01 + 35
        ...
        if language == "pt":
            ...
            extra_divisions = [
                ("Training Needs Analysis", 0, 4),
                ("Antecedent Training Conditions", 5, 14),
                ("Training Methods and Instructional Strategies", 15, 19),
                ("Post-Training Conditions", 20, 26),
            ]
        ...
        add_manual_division_lines(
            ...
            x_label_pos=x_text_base + (xmax - xmin) * 0.06 + 50,
            y_positions=y_positions, line_style='-', color='black', linewidth=1.2,
            label_pad=0.01, rotate=0, fontsize=12,
        ...
        right_margin = x_text_base + (xmax - xmin) * 0.06 + 15
        ax.set_xlim(-100, right_margin)
        ...
        if language == "pt":
            short_labels = [
                ...
                "Adaptação ao nível",
                "Consideração da opinião",
                "Prejuízo ao tempo pessoal",
                "Competitividade no mercado",
                "Motivação para aprender",
                "Incentivo da liderança",
                "Tempo no expediente",
                "Obrigação acima do interesse",
                "Incentivos tangíveis",
                "Reconhecimento",
                "Organização e estrutura",
                "Materiais úteis",
                "Ambiente adequado",
                "Clareza do instrutor",
            ...
            short_labels = [
                "Goal Alignment",               # Conexão com objetivos
                "Support & Resources",          # Apoio e recursos
                "Job Relevance",                # Relevância para a função
                "Level Adaptation",             # Adaptação ao nível
                "Trainee Input",                # Consideração da opinião
                "Personal Time Impact",         # Prejuízo ao tempo pessoal
                "Market Competitiveness",       # Competitividade no mercado
                "Motivation to Learn",          # Motivação para aprender
                "Leadership Encouragement",     # Incentivo da liderança
                "Work Time Allocation",         # Tempo no expediente
                "Obligation over Interest",     # Obrigação acima do interesse
                "Tangible Incentives",          # Incentivos tangíveis
                "Recognition",                  # Reconhecimento
                "Organization & Structure",     # Organização e estrutura
                "Useful Materials",             # Materiais úteis
                "Adequate Environment",         # Ambiente adequado
                "Instructor Clarity",           # Clareza do instrutor
                "Instructor Performance",       # Atuação do instrutor
                "Varied Activities",            # Atividades variadas
                "Soft Skills Focus",            # Foco em Soft Skills
                "Overall Satisfaction",         # Satisfação geral
                "Problem Solving",              # Resolução de problemas
                "Performance Improvement",      # Melhora de desempenho
                "Practical Application",        # Aplicabilidade prática
                "Increased Autonomy",           # Aumento de autonomia
                "Application Support",          # Suporte à aplicação
        ...
        for i, lab in enumerate(percent.index):
            ...
            wrapped = "\n".join(wrap(str(lab), width=40))
            # prefixo em negrito usando mathtext (\bf{})
            prefix = f"$\\bf{{Q{i+1}}}$"
            # prefixed_labels.append(f"{prefix} - {wrapped}")
            prefixed_labels.append(f"{short_labels[i] if i < len(short_labels) else wrapped} - {prefix} ")
        ...
        if language == "pt":
            ...
            ax.set_xlabel("Percentage", fontsize=16, weight="bold", labelpad=2)
        ...
        desired = order  # usar a ordem definida no início da função
        ...
        ax.legend(handles, legend_labels, title="", bbox_to_anchor=(0.3, 1.04), fontsize=14, loc='upper center', ncol=len(legend_labels), frameon=False)
...
def _calculate_likert_mean(idxs, counts, order):
    ...
    order: lista de respostas na ordem da escala (ex: ["Strongly disagree", ..., "Strongly agree"])
    ...
    response_values = {
        order[0]: 1,  # Strongly disagree
        order[1]: 2,  # Disagree
        order[2]: 3,  # Neutral
        order[3]: 4,  # Agree
        order[4]: 5,  # Strongly agree
    }
...
if __name__ == "__main__":
    ...
    language = sys.argv[2] if len(sys.argv) > 2 else "en"  # padrão: inglês
    ...
    res = analyze_likert(df, out_dir=project_root / "output", language=language)
    print({"column": res.get("column"), "total": res.get("total"), "image": res.get("image"), "language": language})
    if res.get("image"):
