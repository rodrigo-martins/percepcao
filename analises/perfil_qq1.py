...
# --- Filtrar apenas respondentes válidos de QQ1 ---
# Excluir nulos
df_qq1 = df[df[qq1_col].notna()].copy()
...
df_qq1 = df_qq1[~df_qq1[qq1_col].str.strip().str.lower().isin(excluir_lower)]

n = len(df_qq1)
print(f'=== Perfil Sociodemográfico dos Respondentes da QQ1 (n={n}) ===\n')


def freq(col, label=None):
    s = df_qq1[col].value_counts()
    pct = (s / n * 100).round(1)
...
# Idade
print(f'--- Idade ---')
print(f'  Média: {df_qq1["Idade"].mean():.1f} anos')
print(f'  Mediana: {df_qq1["Idade"].median():.1f} anos')
print(f'  DP: {df_qq1["Idade"].std():.1f}')
print(f'  Min: {df_qq1["Idade"].min()}, Max: {df_qq1["Idade"].max()}')
print()
...
sup = df_qq1['Instrução'].isin(['Superior completo', 'Superior incompleto']).sum()
pos = df_qq1['Instrução'].isin(['Pós completa', 'Pós incompleta']).sum()
...
senior_count = df_qq1['Nível'].isin(senior_labels).sum()
