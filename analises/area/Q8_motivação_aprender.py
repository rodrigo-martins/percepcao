...
def load_and_prepare(csv_path: Path) -> Tuple[Optional[pd.DataFrame], Optional[Dict]]:
    """
    Carrega CSV e prepara dados para análise de Q8 por área de atuação.
    ...
    col_q8 = find_col(df, " [O que aprendi na experiência de aprendizado me motivou a buscar novos conhecimentos.]")
    
    if col_area is None or col_q8 is None:
        print(f"❌ Colunas não encontradas!")
        print(f"   Área de Atuação: {col_area}")
        print(f"   Q8 (Motivação): {col_q8}")
        return None, None
    
    print(f"✓ Coluna área de atuação: {repr(col_area)}")
    print(f"✓ Coluna Q8: {repr(col_q8)}")
    
    # Preparar dados
    df_clean = df[[col_area, col_q8]].copy()
    df_clean.columns = ["area", "q8"]
    ...
    # Verificar Q8 inválidos ANTES de mapear
    q8_originais = df_clean["q8"].copy()
    q8_unicos = q8_originais.unique()
    print(f"Valores únicos em Q8 (antes de mapear):")
    for val in sorted(q8_unicos, key=lambda x: str(x)):
        count = (q8_originais == val).sum()
        esta_no_map = val in LIKERT_MAP
        status = "✓ VÁLIDO" if esta_no_map else "✗ INVÁLIDO"
        print(f"   {status}: '{val}' (n={count})")
    
    # Mapear Q8 para numérico
    df_clean["q8"] = df_clean["q8"].map(LIKERT_MAP)
    
    q8_invalidos_mask = df_clean["q8"].isna()
    q8_invalidos_count = q8_invalidos_mask.sum()
    
    if q8_invalidos_count > 0:
        print(f"\n❌ Q8 não mapeados (serão removidos): {q8_invalidos_count}")
        print("   Valores originais:")
        valores_invalidos = q8_originais[q8_invalidos_mask].value_counts()
    ...
    # Remover NaNs (Q8 inválidos após mapeamento)
    ...
    for area in AREA_ORDER:
        ...
        if count > 0:
            mean = df_clean[df_clean["area"] == area]["q8"].mean()
            print(f"   {area}: {count} respondentes, média Q8 = {mean:.2f}")
...
def plot_boxplot_by_area(df_clean: pd.DataFrame, out_dir: Path) -> Optional[Path]:
    """
    Cria boxplot de Q8 por área de atuação com paleta laranja.
    Inclui quadro com estatísticas e significância.
    Áreas ordenadas da menos motivada para a mais motivada.
    ...
    means = df_filtrado.groupby("area", observed=True)["q8"].mean()
    
    # Ordenar áreas por média crescente (menos motivado -> mais motivado)
    ...
    # Criar dicionário de cores (menos motivado = mais escuro, mais motivado = mais claro)
    ...
    ax.set_xlabel("Área de Atuação (ordenado por motivação crescente)", fontsize=20, fontweight="bold")
    ax.set_xticklabels(group_labels, fontsize=14, rotation=45, ha="right")
    ax.set_ylabel("Q8 (Motivação para aprender)", fontsize=20, fontweight="bold")
    ...
    for area in df_filtrado["area"].unique():
        subset = df_filtrado[df_filtrado["area"] == area]["q8"].values
        if len(subset) >= 5:
            grupos[area] = subset
...
def analise_motivacao_por_area(csv_path: Optional[Path] = None,
    ...
    for area in AREA_ORDER:
        subset = df_clean[df_clean["area"] == area]["q8"]
        if len(subset) > 0:
            print(f"\n  {area}:")
    ...
    print(f"{'=' * 80}")
    print(f"Hipótese Nula (H0): As distribuições de Q8 são iguais entre todos os grupos de área")
    ...
    for area in df_clean["area"].unique():
        subset = df_clean[df_clean["area"] == area]["q8"].values
    ...
    if len(grupos) < 2:
        ...
        if p_valor < 0.05:
            ...
            print(f"   → Existe diferença significativa na motivação (Q8) entre as áreas de atuação")
        else:
            print(f"\n❌ CONCLUSÃO: Falha em rejeitar H0")
            print(f"   → Não há diferença significativa na motivação (Q8) entre as áreas de atuação")
...
if __name__ == "__main__":
    ...
    result = analise_motivacao_por_area(csv_path=csv_arg, out_dir=out_arg)
