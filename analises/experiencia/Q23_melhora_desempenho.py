...
def load_and_prepare(csv_path: Path) -> Tuple[Optional[pd.DataFrame], Optional[Dict]]:
    """
    Carrega CSV e prepara dados para análise de Q23 por experiência.
    ...
    col_q23 = find_col(df, "melhorou meu desempenho")
    
    if col_exp is None or col_q23 is None:
        print(f"❌ Colunas não encontradas!")
        print(f"   Experiência: {col_exp}")
        print(f"   Q23 (Desempenho): {col_q23}")
        return None, None
    
    print(f"✓ Coluna experiência: {repr(col_exp)}")
    print(f"✓ Coluna Q23: {repr(col_q23)}")
    
    # Preparar dados
    df_clean = df[[col_exp, col_q23]].copy()
    df_clean.columns = ["experiencia", "q23"]
    ...
    # Verificar Q23 inválidos ANTES de mapear
    q23_originais = df_clean["q23"].copy()
    q23_unicos = q23_originais.unique()
    print(f"Valores únicos em Q23 (antes de mapear):")
    for val in sorted(q23_unicos, key=lambda x: str(x)):
        count = (q23_originais == val).sum()
        esta_no_map = val in LIKERT_MAP
        status = "✓ VÁLIDO" if esta_no_map else "✗ INVÁLIDO"
        print(f"   {status}: '{val}' (n={count})")
    
    # Mapear Q23 para numérico
    df_clean["q23"] = df_clean["q23"].map(LIKERT_MAP)
    
    q23_invalidos_mask = df_clean["q23"].isna()
    q23_invalidos_count = q23_invalidos_mask.sum()
    
    if q23_invalidos_count > 0:
        print(f"\n❌ Q23 não mapeados (serão removidos): {q23_invalidos_count}")
        print("   Valores originais:")
        valores_invalidos = q23_originais[q23_invalidos_mask].value_counts()
    ...
    # Remover NaNs (Q23 inválidos após mapeamento)
    ...
    for exp in EXPERIENCIA_ORDER:
        ...
        if count > 0:
            mean = df_clean[df_clean["experiencia"] == exp]["q23"].mean()
            print(f"   {exp}: {count} respondentes, média Q23 = {mean:.2f}")
...
def plot_boxplot_by_experience(df_clean: pd.DataFrame, out_dir: Path) -> Optional[Path]:
    """
    Cria boxplot de Q23 por grupos de experiência com paleta laranja.
    ...
    means = df_clean.groupby("experiencia", observed=True)["q23"].mean()
    ...
    ax.set_ylabel("Q23 (Melhora de desempenho)", fontsize=20, fontweight="bold")
    # ax.set_title(
    #     "Distribuição de Q23 (Desempenho) por Tempo de Experiência",
    #     fontsize=20,
    #     fontweight="bold",
    #     pad=20
    # )
    ...
    for exp in df_clean["experiencia"].unique():
        subset = df_clean[df_clean["experiencia"] == exp]["q23"].values
        if len(subset) >= 5:
            grupos[exp] = subset
...
def analise_desempenho_por_experiencia(csv_path: Optional[Path] = None,
    ...
    for exp in EXPERIENCIA_ORDER:
        subset = df_clean[df_clean["experiencia"] == exp]["q23"]
        if len(subset) > 0:
            print(f"\n  {exp}:")
    ...
    print(f"{'=' * 80}")
    print(f"Hipótese Nula (H0): As distribuições de Q23 são iguais entre todos os grupos de experiência")
    ...
    if len(grupos) < 2:
        ...
        if p_valor < 0.05:
            ...
            print(f"   → Existe diferença significativa na concordância (Q23) entre os grupos de experiência")
        else:
            print(f"\n❌ CONCLUSÃO: Falha em rejeitar H0")
            print(f"   → Não há diferença significativa na concordância (Q23) entre os grupos de experiência")
...
if __name__ == "__main__":
    ...
    result = analise_desempenho_por_experiencia(csv_path=csv_arg, out_dir=out_arg)
