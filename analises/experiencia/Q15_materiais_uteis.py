...
def load_and_prepare(csv_path: Path) -> Tuple[Optional[pd.DataFrame], Optional[Dict]]:
    """
    Carrega CSV e prepara dados para análise de Q15 por experiência.
    ...
    col_q15 = find_col(df, "materiais que recebi")
    
    if col_exp is None or col_q15 is None:
        print(f"❌ Colunas não encontradas!")
        print(f"   Experiência: {col_exp}")
        print(f"   Q15 (Materiais): {col_q15}")
        return None, None
    
    print(f"✓ Coluna experiência: {repr(col_exp)}")
    print(f"✓ Coluna Q15: {repr(col_q15)}")
    
    # Preparar dados
    df_clean = df[[col_exp, col_q15]].copy()
    df_clean.columns = ["experiencia", "q15"]
    ...
    # Verificar Q15 inválidos ANTES de mapear
    q15_originais = df_clean["q15"].copy()
    q15_unicos = q15_originais.unique()
    print(f"Valores únicos em Q15 (antes de mapear):")
    for val in sorted(q15_unicos, key=lambda x: str(x)):
        count = (q15_originais == val).sum()
        esta_no_map = val in LIKERT_MAP
        status = "✓ VÁLIDO" if esta_no_map else "✗ INVÁLIDO"
        print(f"   {status}: '{val}' (n={count})")
    
    # Mapear Q15 para numérico
    df_clean["q15"] = df_clean["q15"].map(LIKERT_MAP)
    
    q15_invalidos_mask = df_clean["q15"].isna()
    q15_invalidos_count = q15_invalidos_mask.sum()
    
    if q15_invalidos_count > 0:
        print(f"\n❌ Q15 não mapeados (serão removidos): {q15_invalidos_count}")
        print("   Valores originais:")
        valores_invalidos = q15_originais[q15_invalidos_mask].value_counts()
    ...
    # Remover NaNs (Q15 inválidos após mapeamento)
    ...
    for exp in EXPERIENCIA_ORDER:
        ...
        if count > 0:
            mean = df_clean[df_clean["experiencia"] == exp]["q15"].mean()
            print(f"   {exp}: {count} respondentes, média Q15 = {mean:.2f}")
...
def plot_boxplot_by_experience(df_clean: pd.DataFrame, out_dir: Path) -> Optional[Path]:
    """
    Cria boxplot de Q15 por grupos de experiência com paleta laranja.
    ...
    means = df_clean.groupby("experiencia", observed=True)["q15"].mean()
    ...
    ax.set_ylabel("Q15 (Materiais úteis)", fontsize=20, fontweight="bold")
    # ax.set_title(
    #     "Distribuição de Q15 (Materiais Úteis) por Tempo de Experiência",
    #     fontsize=20,
    #     fontweight="bold",
    #     pad=20
    # )
    ...
    for exp in df_clean["experiencia"].unique():
        subset = df_clean[df_clean["experiencia"] == exp]["q15"].values
        if len(subset) >= 5:
            grupos[exp] = subset
...
def analise_material_por_experiencia(csv_path: Optional[Path] = None,
    ...
    for exp in EXPERIENCIA_ORDER:
        subset = df_clean[df_clean["experiencia"] == exp]["q15"]
        if len(subset) > 0:
            print(f"\n  {exp}:")
    ...
    print(f"{'=' * 80}")
    print(f"Hipótese Nula (H0): As distribuições de Q15 são iguais entre todos os grupos de experiência")
    ...
    if len(grupos) < 2:
        ...
        if p_valor < 0.05:
            ...
            print(f"   → Existe diferença significativa na concordância (Q15) entre os grupos de experiência")
        else:
            print(f"\n❌ CONCLUSÃO: Falha em rejeitar H0")
            print(f"   → Não há diferença significativa na concordância (Q15) entre os grupos de experiência")
...
if __name__ == "__main__":
    ...
    result = analise_material_por_experiencia(csv_path=csv_arg, out_dir=out_arg)
