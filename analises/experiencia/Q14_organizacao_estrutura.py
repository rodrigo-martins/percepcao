...
def load_and_prepare(csv_path: Path) -> Tuple[Optional[pd.DataFrame], Optional[Dict]]:
    """
    Carrega CSV e prepara dados para análise de Q14 por experiência.
    ...
    col_q14 = find_col(df, "bem organizado")
    
    if col_exp is None or col_q14 is None:
        print(f"❌ Colunas não encontradas!")
        print(f"   Experiência: {col_exp}")
        print(f"   Q14 (Organizado): {col_q14}")
        return None, None
    
    print(f"✓ Coluna experiência: {repr(col_exp)}")
    print(f"✓ Coluna Q14: {repr(col_q14)}")
    
    # Preparar dados
    df_clean = df[[col_exp, col_q14]].copy()
    df_clean.columns = ["experiencia", "q14"]
    ...
    # Verificar Q14 inválidos ANTES de mapear
    q14_originais = df_clean["q14"].copy()
    q14_unicos = q14_originais.unique()
    print(f"Valores únicos em Q14 (antes de mapear):")
    for val in sorted(q14_unicos, key=lambda x: str(x)):
        count = (q14_originais == val).sum()
        esta_no_map = val in LIKERT_MAP
        status = "✓ VÁLIDO" if esta_no_map else "✗ INVÁLIDO"
        print(f"   {status}: '{val}' (n={count})")
    
    # Mapear Q14 para numérico
    df_clean["q14"] = df_clean["q14"].map(LIKERT_MAP)
    
    q14_invalidos_mask = df_clean["q14"].isna()
    q14_invalidos_count = q14_invalidos_mask.sum()
    
    if q14_invalidos_count > 0:
        print(f"\n❌ Q14 não mapeados (serão removidos): {q14_invalidos_count}")
        print("   Valores originais:")
        valores_invalidos = q14_originais[q14_invalidos_mask].value_counts()
    ...
    # Remover NaNs (Q14 inválidos após mapeamento)
    ...
    for exp in EXPERIENCIA_ORDER:
        ...
        if count > 0:
            mean = df_clean[df_clean["experiencia"] == exp]["q14"].mean()
            print(f"   {exp}: {count} respondentes, média Q14 = {mean:.2f}")
...
def plot_boxplot_by_experience(df_clean: pd.DataFrame, out_dir: Path) -> Optional[Path]:
    """
    Cria boxplot de Q14 por grupos de experiência com paleta laranja.
    ...
    means = df_clean.groupby("experiencia", observed=True)["q14"].mean()
    ...
    ax.set_ylabel("Q14 (Organização e estrutura)", fontsize=20, fontweight="bold")
    ...
    for exp in df_clean["experiencia"].unique():
        subset = df_clean[df_clean["experiencia"] == exp]["q14"].values
        if len(subset) >= 5:
            grupos[exp] = subset
...
def analise_organizado_por_experiencia(csv_path: Optional[Path] = None,
    ...
    for exp in EXPERIENCIA_ORDER:
        subset = df_clean[df_clean["experiencia"] == exp]["q14"]
        if len(subset) > 0:
            print(f"\n  {exp}:")
    ...
    print(f"{'=' * 80}")
    print(f"Hipótese Nula (H0): As distribuições de Q14 são iguais entre todos os grupos de experiência")
    ...
    if len(grupos) < 2:
        ...
        if p_valor < 0.05:
            ...
            print(f"   → Existe diferença significativa na concordância (Q14) entre os grupos de experiência")
        else:
            print(f"\n❌ CONCLUSÃO: Falha em rejeitar H0")
            print(f"   → Não há diferença significativa na concordância (Q14) entre os grupos de experiência")
...
if __name__ == "__main__":
    ...
    result = analise_organizado_por_experiencia(csv_path=csv_arg, out_dir=out_arg)
