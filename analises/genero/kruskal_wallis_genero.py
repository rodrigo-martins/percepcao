...
GENERO_ORDER = ["Masculino", "Feminino"]
...
def load_and_prepare(csv_path: Path) -> Tuple[Optional[pd.DataFrame], Optional[List[str]]]:
    """
    Carrega CSV, identifica coluna de gênero e colunas Likert.
    Filtra apenas Masculino e Feminino.
    ...
    print(f"\n📊 Distribuição de gênero (antes do filtro):")
    for gen, count in df_clean["genero"].value_counts().items():
        print(f"   {gen}: {count}")

    df_clean = df_clean[df_clean["genero"].isin(GENERO_ORDER)].copy()
    ...
    print(f"\n✓ Após filtro (Masculino + Feminino): {len(df_clean)} respondentes")
    for gen in GENERO_ORDER:
        n = (df_clean["genero"] == gen).sum()
        print(f"   {gen}: {n}")
...
def run_kruskal_all(df_clean: pd.DataFrame,
    ...
    """
    Executa Kruskal-Wallis para cada questão Likert entre Masculino e Feminino.
    Retorna DataFrame com colunas: questao, label, H, p_valor, significancia.
    """
    results = []

    for i, col in enumerate(likert_cols):
        q_label = f"Q{i + 1}"
        subset = df_clean[["genero", col]].dropna()

        grupo_masc = subset[subset["genero"] == "Masculino"][col].values
        grupo_fem = subset[subset["genero"] == "Feminino"][col].values

        if len(grupo_masc) < 5 or len(grupo_fem) < 5:
            results.append({
                "questao": q_label,
                "coluna": col,
                "n_masc": len(grupo_masc),
                "n_fem": len(grupo_fem),
                "media_masc": np.nan,
                "media_fem": np.nan,
                "H": np.nan,
        ...
        h_stat, p_valor = stats.kruskal(grupo_masc, grupo_fem)
        ...
        results.append({
            ...
            "n_masc": len(grupo_masc),
            "n_fem": len(grupo_fem),
            "media_masc": grupo_masc.mean(),
            "media_fem": grupo_fem.mean(),
            "H": h_stat,
            "p_valor": p_valor,
            "significancia": sig,
        })
...
def plot_summary(results_df: pd.DataFrame, out_dir: Path) -> Optional[Path]:
    ...
    # Filtrar questões com resultado válido
    ...
    # Cores: laranja para significativo, cinza claro para não significativo
    cores = [BASE_ORANGE if p < 0.05 else "#d0d0d0" for p in p_valores]

    bars = ax.barh(y_pos, p_valores, color=cores, edgecolor="white", height=0.7)

    # Linha de corte α = 0.05
    ax.axvline(x=0.05, color="red", linestyle="--", linewidth=2, label="α = 0.05")

    # Adicionar p-valor como texto ao lado de cada barra
    ...
    ax.set_title("Kruskal-Wallis: Masculino vs Feminino", fontsize=16, fontweight="bold")
    ax.legend(fontsize=12, loc="lower right")
    ax.set_xlim(0, max(p_valores.max() * 1.3, 0.1))
    ax.grid(axis="x", alpha=0.3, linestyle="--")

    plt.tight_layout()
    out_path = out_dir / "kruskal_wallis_genero_resumo.png"
...
def plot_boxplot_significativo(df_clean: pd.DataFrame, col: str,
    ...
    Segue padrão visual do projeto: paleta laranja, médias como bolinhas pretas.
    """
    ...
    subset = df_clean[["genero", col]].dropna()
    orange_palette = _orange_shades(BASE_ORANGE, len(GENERO_ORDER))
    color_dict = {gen: orange_palette[i] for i, gen in enumerate(GENERO_ORDER)}
    ...
    means = subset.groupby("genero")[col].mean()
    for pos, gen in enumerate(GENERO_ORDER):
        mean_val = means[gen]
    ...
    group_labels = []
    for gen in GENERO_ORDER:
        n = (subset["genero"] == gen).sum()
        group_labels.append(f"{gen}\n(n={n})")
    ...
    quadro_text = (
        ...
        f"{'─' * 32}\n"
        f"n = {len(grupo_masc) + len(grupo_fem)}\n"
        f"H = {h_stat:.4f}\n"
        f"p = {p_valor:.6f}\n"
    ...
    # Extrair texto curto da questão para o título do eixo Y
    col_short = col.strip().strip("[]").strip()
    if len(col_short) > 80:
        col_short = col_short[:77] + "..."

    ax.set_xlabel("Gênero", fontsize=20, fontweight="bold")
    ...
    plt.tight_layout()
    out_path = out_dir / f"kruskal_genero_{q_label}.png"
    fig.savefig(out_path, dpi=300, bbox_inches="tight", facecolor="white")
    plt.close(fig)
...
def print_ranking(results_df: pd.DataFrame) -> None:
    ...
    print("\n" + "=" * 70)
    print("📊 RANKING DE SIGNIFICÂNCIA - GÊNERO (Masculino vs Feminino)")
    print("=" * 70)
    ...
    if sig.empty:
        ...
        for rank, (_, row) in enumerate(sig.iterrows(), 1):
            print(f"{rank:2d}. {row['questao']:3s} | "
                  ...
                  f"Masc={row['media_masc']:.2f} Fem={row['media_fem']:.2f}")

    # Também mostrar as não-significativas
...
def analise_kruskal_genero(csv_path: Optional[Path] = None,
                           out_dir: Optional[Path] = None) -> Dict[str, Any]:
    """
    Pipeline completo: carrega, testa Kruskal-Wallis para todas as questões
    entre Masculino e Feminino, gera gráfico resumo e boxplots significativos.
    """
    project_root = Path(__file__).resolve().parents[2]

    csv_path = Path(csv_path) if csv_path is not None else project_root / "data" / "tratado.csv"
    out_dir = Path(out_dir) if out_dir is not None else project_root / "output" / "kruskal_genero"
...
if __name__ == "__main__":
    ...
    result = analise_kruskal_genero(csv_path=csv_arg, out_dir=out_arg)
