...
def plot_boxplot_states(df_clean: pd.DataFrame, out_dir: Path) -> Optional[Path]:
    ...
    Estados ordenados da menor para maior média (esquerda → direita).
    """
    ...
    # Calcular médias por estado e ordenar (menor → maior)
    state_means = df_clean.groupby("estado")["q10"].mean().sort_values(ascending=True)
    state_order = state_means.index.tolist()
    
    print(f"\n📈 Ordem dos estados (menor → maior média):")
