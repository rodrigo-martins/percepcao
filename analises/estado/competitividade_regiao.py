...
def plot_boxplot_regions(df_clean: pd.DataFrame, out_dir: Path) -> Optional[Path]:
    ...
    region_means = df_clean.groupby("Regiao")["q7"].mean().sort_values(ascending=False)
    ...
    for pos, regiao in enumerate(region_order):
        ...
        # Primeiras regiões (mais escuras) = branco, últimas 2 (mais claras) = preto
        text_color = "white" if pos < len(region_order) - 2 else "black"
    ...
    ax.set_title(
        "Distribuição por Região: Q7 (Manter competitivo)",
        fontsize=18,
        fontweight="bold",
        pad=20
    )
    ...
    # Salvar figura
    out_path = out_dir / "region_analysis_Q7.png"
